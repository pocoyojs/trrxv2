...
function createToast(msg, color) {
    ...
    if (color === 'red') {
        t.className = `fixed inset-0 z-[10000] flex items-center justify-center bg-black/95 p-4 animate-in`;
        t.innerHTML = `
            <div class="bg-red-600 p-8 rounded-[2rem] shadow-2xl max-w-5xl w-full border-2 border-white/20">
                <h1 class="text-2xl font-black text-white mb-4 uppercase italic">ERRO COMPLETO DETECTADO</h1>
                <div class="bg-black/60 p-6 rounded-xl text-left border border-white/10">
                    <code class="text-sm font-mono text-yellow-300" style="white-space: pre-wrap; word-break: break-all; display: block;">${msg}</code>
                </div>
                <button onclick="this.parentElement.parentElement.remove()" class="mt-6 bg-white text-red-600 px-8 py-3 rounded-full font-black uppercase text-xs">Fechar e voltar ao console (CTRL+SHIFT+I)</button>
            </div>
        `;
        t.style.opacity = '1';
    } else {
        t.className = `fixed bottom-10 right-10 green-gradient px-8 py-4 rounded-2xl shadow-2xl z-[100] font-black italic uppercase text-xs text-white transition-all duration-500`;
        t.innerText = msg;
        t.style.opacity = '1';
